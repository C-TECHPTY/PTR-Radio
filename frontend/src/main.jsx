...
const nav = [
  ...
  ['Relojes Musicales', Clock3], ['Cartuchera', Zap], ['AzuraCast', Radio], ['Configuración', Settings],
];
...
function App() {
  ...
  return <div className="flex min-h-screen"><Sidebar active={active} setActive={setActive} logout={logout} /><div className="min-w-0 flex-1 lg:pl-64"><Header title={active} />{active === 'Dashboard' ? <Dashboard /> : <ModulePage title={active} />}</div></div>;
}
...
function Dashboard() {
  const [data, setData] = useState(null); const [mode, setMode] = useState('autodj');
  useEffect(() => { fetch(`${API}/dashboard`).then(r => r.ok ? r.json() : Promise.reject()).then(setData).catch(() => setData({ azuracast: { online: false }, listeners: 0, nowPlaying: { title: 'Sin conexión', artist: 'Configure el backend', elapsed: 0, duration: 1 }, next: { title: 'Sin datos', artist: '—' }, cartwall: Array.from({length:20}, (_,i)=>({id:i+1,label:`CART ${String(i+1).padStart(2,'0')}`,color:colors[i%5]})) })); }, []);
  if (!data) return <div className="grid min-h-[60vh] place-items-center text-cyan-400"><Activity className="animate-pulse" /></div>;
  const progress = Math.min(100, Math.round(data.nowPlaying.elapsed / data.nowPlaying.duration * 100)); const remaining = Math.max(0, data.nowPlaying.duration - data.nowPlaying.elapsed);
  return <main className="space-y-6 p-5 pb-10 md:p-8"><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat icon={Radio} label="Estado AzuraCast" value={data.azuracast.online ? 'Conectado' : 'Sin configurar'} detail={data.azuracast.station || 'API oficial'} ok={data.azuracast.online} /><Stat icon={Gauge} label="Oyentes actuales" value={data.listeners} detail="En tiempo real" /><Stat icon={Disc3} label="Modo de emisión" value={mode === 'autodj' ? 'AutoDJ' : 'Manual'} detail="Operación activa" /><Stat icon={Activity} label="Señal" value={data.azuracast.online ? 'Estable' : 'Local'} detail="Monitoreo continuo" /></section>
    <section className="grid gap-6 xl:grid-cols-[1.65fr_1fr]"><NowPlaying song={data.nowPlaying} progress={progress} remaining={remaining} /><div className="space-y-6"><Next song={data.next} /><Mode mode={mode} setMode={setMode} /></div></section>
    <Cartwall items={data.cartwall} /></main>;
}
...
function NowPlaying({ song, progress, remaining }) { return <div className="panel overflow-hidden"><div className="flex items-center justify-between border-b border-line p-5"><div className="flex items-center gap-2 text-sm font-bold"><Volume2 size={17} className="text-cyan-400" /> AL AIRE AHORA</div><span className="rounded-full bg-rose-500/10 px-3 py-1 text-[10px] font-bold tracking-widest text-rose-400"><span className="mr-1 animate-pulse">●</span> EN VIVO</span></div><div className="flex flex-col gap-6 p-6 sm:flex-row sm:items-center"><div className="grid aspect-square w-full max-w-44 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-cyan-500/20 via-violet-500/20 to-slate-900 shadow-glow"><Music2 size={60} className="text-cyan-300/70" /></div><div className="min-w-0 flex-1"><div className="label">Reproduciendo</div><h2 className="mt-2 truncate text-3xl font-extrabold tracking-tight">{song.title}</h2><p className="mt-1 text-lg text-slate-400">{song.artist}</p><div className="mt-8 h-2 overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-500" style={{width:`${progress}%`}} /></div><div className="mt-3 flex justify-between font-mono text-xs text-slate-500"><span>{format(song.elapsed)}</span><span className="text-cyan-300">-{format(remaining)}</span><span>{format(song.duration)}</span></div></div></div></div>; }
function Next({ song }) { return <div className="panel p-5"><div className="label flex items-center gap-2"><ListMusic size={15} /> Próxima canción</div><div className="mt-4 flex items-center gap-4"><div className="grid h-14 w-14 place-items-center rounded-xl bg-violet-500/10 text-violet-300"><Album /></div><div className="min-w-0"><div className="truncate font-bold">{song.title}</div><div className="truncate text-sm text-slate-500">{song.artist}</div></div><Play className="ml-auto text-slate-600" size={18} /></div></div>; }
function Mode({ mode, setMode }) { return <div className="panel p-5"><div className="label mb-4 flex items-center gap-2"><SlidersHorizontal size={15} /> Control de emisión</div><div className="grid grid-cols-2 gap-2">{[['autodj','AutoDJ',Sparkles],['manual','Manual',Mic2]].map(([id,label,Icon])=><button key={id} onClick={()=>setMode(id)} className={`rounded-xl border p-4 text-left transition ${mode===id?'border-cyan-400/40 bg-cyan-400/10 text-cyan-300':'border-line bg-slate-950/30 text-slate-500'}`}><Icon size={18}/><div className="mt-3 text-sm font-bold">{label}</div></button>)}</div></div>; }
function Cartwall({ items }) { return <section className="panel p-5"><div className="mb-4 flex items-center justify-between"><div><div className="label">Acceso instantáneo</div><h2 className="mt-1 font-bold">Cartuchera</h2></div><button className="rounded-lg border border-line p-2 text-slate-500"><Settings size={16}/></button></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4 md:grid-cols-5 xl:grid-cols-10">{items.map((item,i)=><button key={item.id} className={`group min-h-20 rounded-xl border p-3 text-left transition hover:-translate-y-0.5 hover:brightness-125 ${colorClasses[item.color] || colorClasses.cyan}`}><Zap size={15}/><div className="mt-3 truncate text-[11px] font-extrabold">{item.label}</div><div className="mt-1 text-[9px] opacity-50">F{(i%10)+1}</div></button>)}</div></section>; }
